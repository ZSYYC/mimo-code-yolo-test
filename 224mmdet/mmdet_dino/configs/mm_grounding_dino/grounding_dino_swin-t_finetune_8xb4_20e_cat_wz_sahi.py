...
model = dict(bbox_head=dict(
        ...
        loss_cls=dict(
            ...
            gamma=2.0,
            alpha=0.75,      # 提高正样本权重
            loss_weight=2.0  # 分类比 bbox 更重要
        ),

        loss_bbox=dict(
            type='L1Loss',
            loss_weight=5.0  # tiny bbox 要重
        ),
...
dataset_10000 = dict(
    ...
    pipeline=train_pipeline,
    filter_cfg=dict(filter_empty_gt=False, min_size=32),
    ann_file=data_root + 'annotations_sahi/instances_train_sahi.json',
    data_prefix=dict(img=data_root + 'sahi_train/'))
# dataset_10000=dict(
#     type='CocoDataset',
#     data_root=data_root,
#     metainfo=metainfo,
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        return_classes=True,
        data_root=data_root,
        ann_file=data_root + 'annotations_sahi/instances_val_sahi.json',
        data_prefix=dict(img=data_root + 'sahi_val/')))

test_dataloader = val_dataloader

val_evaluator = dict(ann_file=data_root + 'annotations_sahi/instances_val_sahi.json')
...
optim_wrapper = dict(
    optimizer=dict(lr=0.00005, type='AdamW', weight_decay=0.0001),
...
load_from = '/home/star/yyc/mmdet_dino/grounding_dino_swin-t_pretrain_obj365_goldg_grit9m_v3det_20231204_095047-b448804b.pth'