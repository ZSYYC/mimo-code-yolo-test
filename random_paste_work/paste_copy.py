...
src_img = '/home/yaoteam/yaoteam/yyc/random_paste_work/虫情测报灯图片/part3' #代扩增目标图片
src_xml = '/home/yaoteam/yaoteam/yyc/random_paste_work/虫情测报灯图片/xml3'
img_path = '/home/yaoteam/yaoteam/yyc/random_paste_work/crop_output_bbfshfsyechan_yyc'  # 小图路径

target_root = '/home/yaoteam/yaoteam/yyc/random_paste_work/虫情测报灯图片/output/3' #输出目录
...
def get_target_files(prefix_path):
    target_img = {}
    for cls in clses:
        files = glob.glob(os.path.join(prefix_path,cls,'*.png'))    # 此处被修改本来jpg
        target_img[cls] = files
    return target_img

def anchor_generator(ws, hs, img_w, img_h, stride_w, stride_h, dtype=torch.int32):
    """
...
def custom_box_iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    """
    返回两组框之间的自定义IoU值，计算为交集面积除以较小目标框的面积。
    ...
    返回:
    - Tensor[N, M]: 包含boxes1中每个框与boxes2中每个框的自定义IoU值的NxM矩阵。
    """
    area1 = (boxes1[:, 2] - boxes1[:, 0]) * (boxes1[:, 3] - boxes1[:, 1])
    area2 = (boxes2[:, 2] - boxes2[:, 0]) * (boxes2[:, 3] - boxes2[:, 1])

    lt = torch.max(boxes1[:, None, :2], boxes2[:, :2])  # [N,M,2]，每对框的左上角的最大值
    rb = torch.min(boxes1[:, None, 2:], boxes2[:, 2:])  # [N,M,2]，每对框的右下角的最小值

    wh = (rb - lt).clamp(min=0)  # [N,M,2]，交集区域的宽和高
    inter_area = wh[:, :, 0] * wh[:, :, 1]  # [N,M]，交集区域的面积

    smaller_area = torch.min(area1[:, None], area2)  # [N,M]，每对框中较小框的面积
    custom_iou = inter_area / smaller_area  # 计算自定义IoU值

    return custom_iou

def single_cls_location(gt, anchors, iou_thr):   
    overlaps = custom_box_iou(gt, anchors)
    max_overlaps, argmax_overlaps = overlaps.max(dim=0)
    # print(max_overlaps)
    assigned_anchor_inds = max_overlaps <= iou_thr
    # print(assigned_anchor_inds)
...
def overlay_image(large_img, small_img, x_offset, y_offset):
    """将小图粘贴到大图上的指定位置，保留透明度信息。"""
...
def main():
    ...
    while not is_enough:
        ...
        for xml_file in tqdm(xml_files):
            ...
            if image.shape[2] == 3:  # 如果是不带 Alpha 通道的图片，转换为带 Alpha 通道
            ...
            for idx, cls in enumerate(clses):
                ...
                if target_file is not None:
                    ...
                    small_img = cv2.imread(target_file, cv2.IMREAD_UNCHANGED)
                    ...
                    # 检查 bboxes 是否为空
                    if bboxes.numel() > 0:
                        location = single_cls_location(bboxes, anchors,0.5)  # 调节重叠率
                    else:
                        # bboxes 为空时，随机选择一个位置
                        if anchors.numel() > 0:
                    ...
                    overlay_image(image, small_img, x1, y1)
...
if __name__ == '__main__':
    ...
    # 需要添加的类别与添加数量
    clses = ['y1','y2','yechan']
    # clses = ['SBPH','y1','y2','cl','dhc','ehm','hzlc','dm','dz']
